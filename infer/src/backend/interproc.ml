...
let forward_tabulate tenv wl =
  ...
  let handle_exn curr_node exn =
    ...
    Deferred_errors.save (exn,curr_node);
...
let initiate_footpatch () =
  begin try
      Sys.getenv "GLOBAL_REPAIR" |> ignore;
      Candidate.Top_down.load_global_store ();
    with Not_found -> ()
  end;
  ...
  Footpatch_log.log
  @@ Format.sprintf "SEARCH STORE SIZE %d"
  @@ Hashtbl.length store;

  (* Print so we can see how clusters are processed in parallel *)

  Format.printf "SEARCH STORE SIZE %d@." @@ Hashtbl.length store;

  (*
  let all_candidates =
    Hashtbl.fold
      (fun _ value (acc : Candidate.Top_down.t list) ->
         value@acc)
      store
      []
  in

  IList.iter Candidate.Top_down.log all_candidates;
  *)

  let process_error (exn,curr_node) =
    let loc,err_desc, _, _, _, _, _ = Exceptions.recognize_exception exn in
    let node_loc = Cfg.Node.get_loc curr_node in

    begin
      match Localise.to_string loc with
      | "NULL_DEREFERENCE" ->

          Footpatch_log.log
          @@ F.sprintf "[+] Found error: %s"
          @@ Localise.to_string loc;

          let err_desc = Utils.pp_to_string Localise.pp_error_desc err_desc in

          Footpatch_log.log
          @@ F.sprintf "[+] Patchable error: [%s]:[%d] :[%s]"
            (DB.source_file_to_string node_loc.Location.file)
            node_loc.Location.line
            err_desc;

          let pvar = Footpatch_utils.pvar_of_null_deref_err_desc err_desc in
          Footpatch.run curr_node "NULL_DEREF" pvar None
      ...
      | "RESOURCE_LEAK" ->
          ...
          let pvar_result =
            try
              Footpatch_utils.get_pvar_and_typ_from_resource_err curr_node err_desc
            with | _ ->
              Footpatch_log.log
              @@ Format.sprintf "[-] Could not get pvar";
              None
          in

          begin
            match pvar_result with
            | Some (pvar,typ) ->
                let pvar = Footpatch_utils.normalize_pvar_name pvar in
                Footpatch.run curr_node "RESOURCE_LEAK" pvar (Some typ)
            | None ->
                Footpatch_log.log
                @@ Format.sprintf
                  "\t[-] Could not find pvar name or type of error"
          end
      ...
      | "MEMORY_LEAK" ->
          ...
          begin
            (* resolve pvar and type *)
            match
              Footpatch_utils.get_pvar_and_typ_from_mem_alloc_err
                curr_node
                err_desc
            with
            | Some (pvar,typ) ->
                let pvar = Footpatch_utils.normalize_pvar_name pvar in
                Footpatch.run curr_node "MEMORY_LEAK" pvar (Some typ)
            | None ->
                Footpatch_log.log
                @@ F.sprintf "[-] Could not get pvar name and type"
          end
      | _ -> ()
    end
  in

  Footpatch_log.log @@ Format.sprintf "SIZE OF ERRORS: %d" @@
  IList.length (Deferred_errors.get ());
  Format.printf "SIZE OF ERRORS: %d@." @@ IList.length (Deferred_errors.get());
  IList.iter process_error (Deferred_errors.get ())
