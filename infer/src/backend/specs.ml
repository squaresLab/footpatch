...
let load_summary_to_spec_table proc_name =
  let add summ origin =
    L.out "Adding summary from a load_summary_to_spec_table@.%!";
  ...
  | Some summ ->
      try
        Sys.getenv "IDGAF" |> ignore;
        (* loads summary found on disc into memory *)
        (* introduces nondetermnistic parallel runs on CG *)
        add summ DB.Res_dir
      with
      (* by default, pretend we can't find a summary to stop nondeterministic
         parallel runs *)
      | Not_found -> false
