...
let analyze_cluster cluster_num (cluster: Cluster.t) => {
  ...
  /*Footpatch_log.log "In analyze_cluster";*/
...
let process_cluster_cmdline fname =>
  ...
  | Some (nr, cluster) =>
    /*Footpatch_log.log @@ Format.sprintf "%s" (Utils.pp_to_string Cluster.pp_cluster (nr, cluster));*/
...
let () = {
  ...
  switch Config.cluster_cmdline {
  | Some fname =>
    Footpatch_log.log @@ Format.sprintf "Doing cluster %s" fname;
  ...
  | None =>
    ...
    if (Config.makefile_cmdline != "") {
      ...
      /* when no cluster is specified, and we are not asked to create a makefile */
      IList.iteri (fun i cluster => analyze_cluster i cluster) clusters;
      /*Interproc.initiate_footpatch ();*/
      L.stdout "Analysis finished in %as@." pp_elapsed_time ()
    };
    output_json_makefile_stats clusters
  };
  /* Environment variable to dump: DUMP_CANDS */
  Candidate.Top_down.dump_candidates_to_db ();
  /* footpatch is initiated after running analyze */
  L.out "Initiating footpatch in InferAnalyze@.";
  Interproc.initiate_footpatch ()
};
